  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="relative bg-white dark:bg-slate-800 w-full max-w-[560px] rounded-[3rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-500 border border-white/20 dark:border-slate-700 max-h-[90vh] overflow-y-auto">
        ...
        {!paymentMethod ? (
          <div className="pt-10 pb-12 px-8 md:px-12 flex flex-col items-center">
            <div className="text-center mb-10">
              <div className="size-20 bg-primary/10 rounded-3xl flex items-center justify-center text-primary mx-auto mb-6 transform -rotate-6 hover:rotate-0 transition-all shadow-inner">
                <span className="material-symbols-outlined text-4xl filled">payments</span>
              </div>
              <h2 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white mb-3 tracking-tight">Payment Options</h2>
            ...
            <div className="grid gap-4 w-full">
              <button 
                ...
                className="group relative flex items-center gap-6 p-6 rounded-[2rem] border-2 border-slate-100 dark:border-slate-700 hover:border-primary hover:bg-primary/5 transition-all text-left active:scale-[0.98]"
              >
                <div className="size-16 rounded-2xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center text-slate-400 group-hover:text-primary transition-colors">
                  <span className="material-symbols-outlined text-3xl">account_balance</span>
                </div>
                <div className="flex-1">
                  <h4 className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tight">Bank Transfer</h4>
                  <p className="text-xs text-slate-500 font-bold mt-1 uppercase tracking-widest">Confirmation takes within 24-48hrs</p>
                </div>
              ...
              >
                ...
                <div className="flex-1">
                  <h4 className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tight">Pay Online</h4>
                  <p className="text-xs text-primary font-bold mt-1 uppercase tracking-widest">Instant payment confirmation</p>
                </div>
                <span className="material-symbols-outlined text-slate-300 group-hover:text-primary transition-colors">chevron_right</span>
              </button>
            </div>
          ...
          <div className="pt-10 pb-8 px-8 md:px-12 flex flex-col items-center text-center relative">
            ...
            <div 
              className="w-full bg-slate-50 dark:bg-slate-900 p-8 rounded-[2rem] border-2 border-primary/20 shadow-sm mb-8 group cursor-copy active:scale-[0.98] transition-all relative overflow-hidden"
              onClick={() => {
            ...
            <div className="w-full mb-8">
              <button 
                ...
                className={`w-full group p-6 rounded-3xl border-2 border-dashed transition-all flex items-center justify-center gap-4 ${receiptUrl ? 'bg-green-500/10 border-green-500' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-700 hover:border-primary'}`}
              >
                {receiptUrl ? (
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left overflow-hidden">
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest truncate">{receiptFile?.name}</p>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-sm font-black text-slate-500 group-hover:text-primary transition-colors uppercase tracking-widest">
                        {receiptProgress > 0 ? 'Uploading...' : 'Upload Transfer Receipt'}
                      </span>
                  </div>
                )}
              </button>
            ...
            <div className="w-full flex flex-col gap-4">
              <button 
                ...
                className={`w-full text-white font-black text-xl py-5 rounded-[1.5rem] shadow-xl transition-all flex items-center justify-center gap-3 active:scale-95 hover:-translate-y-1 bg-primary shadow-primary/30 ${(!receiptUrl || (receiptProgress > 0 && receiptProgress < 100)) ? 'opacity-50 grayscale cursor-not-allowed hover:translate-y-0 active:scale-100' : ''}`}
              >
                  {receiptUrl ? 'Confirm & Finish' : "I've sent the funds"}
                  <span className="material-symbols-outlined">task_alt</span>
              </button>
              
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] py-4">
